...
public:
    ...
    bool initDatabase();
    bool createTables();
    bool isFavorite(int userId, int articleId);

        bool removeFavorite(int userId, int articleId);
    // 文章操作
    ...
    // 初始化时创建收藏表
        bool initFavoriteTable();
        // 添加收藏记录
        bool addFavorite(int userId, int articleId);
        // 获取指定用户的收藏文章列表
        QList<Article> getFavoriteArticles(int userId);
        // 检查是否已收藏（防止重复收藏）
        bool isFavorited(int userId, int articleId);
    // 统计
